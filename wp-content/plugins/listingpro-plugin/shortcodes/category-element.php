...
vc_map( array(
	...
	"params"                    => array(
		...
		array(
            ...
            'dependency'  => array(
                'element' => 'catstyles',
                'value'   => array('cat_abstracted', 'cat_abstracted_2', 'cat_boxed', 'cat_boxed_2', 'cat_grid_abstracted', 'cat_ab_grid_abstracted' )
            ),
            'value' => $cats
        ),
        array(
            "type"        => "dropdown",
            "class"       => "",
            "heading"     => esc_html__("Select Parent Category","js_composer"),
            "param_name"  => "display_main_cats",
            'value' => $cats_parent,
            'dependency'  => array(
                'element' => 'catstyles',
        ...
        array(
            'type' => 'checkbox',
            'heading' => __( 'Select Child Category', 'js_composer' ),
            'param_name' => 'child_category_ids',
            'description' => __( 'Check the checkbox' ),
            'value' => $first_child_term_Arr,
            'dependency'  => array(
                'element' => 'catstyles',
                'value'   => 'cat_slider_style'
            ),
...
function listingpro_shortcode_listing_cats($atts, $content = null) {
	...
	$output .= '</div>';
        ...
        if($catstyles == 'cat_slider_style') {
            $category_ids   =   $child_category_ids;
            $parent_term    =   get_term_by( 'id', $display_main_cats, 'listing-category' );
            $output .=  '<h3 class="lp-main-cate-name text-center">'.$parent_term->name.'</h3>';
	...
	$allLocations = get_terms( 'listing-category',$ucat);
        if( $catstyles == 'cat_abstracted_2' )
        {
            ...
            foreach ( $allLocations as $category )
            {
                ...
                {
                    if(empty($imgurl) ){
                        $catImg = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=465&h=375';
                        $catImg2 = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=500&h=300';
                    }
                    else{
                ...
                if( $cat_counter == 2 )
                {
                    ...
                    if(empty($imgurl) ){
                        $catImg = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=335&h=390';
                        $catImg2 = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=500&h=300';
                    }
                    else{
                ...
                if ( $cat_counter == 3 ){
                    if(empty($imgurl) ){
                        $catImg = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=335&h=190';
                        $catImg2 = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=500&h=300';
                    }
                    else{
                ...
                {
                    if(empty($imgurl) ){
                        $catImg = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=585&h=375';
                        $catImg2 = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=500&h=300';
                    }
                    else{
                        $catImg =   aq_resize( $category_image, '336', '182', true, true, true);
                ...
                if( $cat_counter == 6 || $cat_counter == 7 )
                {
                    ...
                    if(empty($imgurl) ){
                        $catImg = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=292&h=375';
                        $catImg2 = 'https://placeholdit.imgix.net/~text?txtsize=33&455&w=500&h=300';
                    }
                    else{
		...
		foreach($allLocations as $category) {
            ...
            foreach($allLocations as $category) {
                ...
                if( !empty($cat_image_id) ){
                    ...
                    if(empty($imgurl) ){
                        $catImg = 'https://placeholdit.imgix.net/~text?txtsize=33&txt=272%C3%97150&w=272&h=150';
                    }
                    else{
                        $catImg = $imgurl;
                    }
